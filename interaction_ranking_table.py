...
def sort_interactions(df):
    cols = pd.MultiIndex.from_product([[pc for pc in df.columns[:-1]], ['interactions', 'weights']])
    sorted_df = pd.DataFrame(index=range(len(df.index)), columns=cols)

    for pc in df.columns[:-1]:
        sorted_ = df[['interactions', pc]].copy()
    ...
    sorted_df.dropna(how='all', inplace=True, axis=1)
    return sorted_df


def process_data(dir_path, two_body, three_body, one_body=False):  # , principal_component
    df = pd.DataFrame()
    if one_body:
        ...
        three_df = three_df.reindex(three_df.columns[three_df.columns != 'interactions'].union(['interactions']),
                                    axis=1)
        ...
        three_df = sort_interactions(three_df)
        three_df['body'] = 'three_body'
        df = pd.concat([df, two_df, three_df], axis=0)
        df.set_index(['body', df.index], inplace=True)
